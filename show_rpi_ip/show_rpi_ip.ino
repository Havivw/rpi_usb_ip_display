...
void typeHostCommand() {
    ...
    // One-liner: find the RP2350 CDC port by USB VID (2e8a) and send hostname + IP info back only to that port, then exit the terminal
    const char *cmd = "for d in /dev/ttyACM*; do [ -e \"$d\" ] || continue; B=$(basename \"$d\"); VID_PATH=\"/sys/class/tty/$B/device/../idVendor\"; VID=$(cat \"$VID_PATH\" 2>/dev/null || true); [ \"$VID\" = \"2e8a\" ] || continue; [ -w \"$d\" ] || continue; ( hostname; ip -o -4 addr show dev wlan0 || true; ip -o -4 addr show dev eth0 || true; ip -o -4 addr show ) > \"$d\"; done; exit";
