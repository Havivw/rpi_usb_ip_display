// Linux version - IP and MAC address display for RP2350 + Waveshare 1.47" LCD
// Requires the host system to be logged in to a graphical desktop
//
// LANGUAGE NOTE:
// The Ctrl+Alt+T shortcut works on most Linux desktop environments (GNOME, KDE, XFCE).
// The shell commands (ip, hostname) are language-independent.
// If Ctrl+Alt+T doesn't open a terminal on your system, you may need to configure it
// in your desktop environment's keyboard shortcuts settings.
...
// Extract MAC address from "link/ether xx:xx:xx:xx:xx:xx" format
String extractMac(const String &src) {
    int idx = src.indexOf("link/ether ");
    if (idx < 0) return "";
    idx += 11; // length of "link/ether "
    
    // Skip any extra spaces
    while (idx < src.length() && src[idx] == ' ') idx++;
    
    // Extract MAC (17 chars: aa:bb:cc:dd:ee:ff)
    String mac;
    for (int i = idx; i < src.length() && mac.length() < 17; i++) {
        char c = src[i];
        bool isMacChar = (c >= '0' && c <= '9') || (c >= 'a' && c <= 'f') || (c >= 'A' && c <= 'F') || c == ':';
        if (isMacChar) {
            mac += c;
        } else {
            break;
    ...
    // Validate it looks like a MAC (has colons)
    if (mac.length() >= 11 && mac.indexOf(':') > 0) {
        return mac;
    }
    return "";
}
...
void parseIpLine(const String &line) {
    ...
    // Check for MAC address in link lines (ip -o link output)
    String mac = extractMac(noAnsi);
    if (mac.length() > 0 && iface.length() > 0) {
...
void typeHostCommand() {
    ...
    // One-liner: find the RP2350 CDC port by USB VID (2e8a) and send hostname + IP + MAC info back only to that port, then exit the terminal
    const char *cmd = "for d in /dev/ttyACM*; do [ -e \"$d\" ] || continue; B=$(basename \"$d\"); VID_PATH=\"/sys/class/tty/$B/device/../idVendor\"; VID=$(cat \"$VID_PATH\" 2>/dev/null || true); [ \"$VID\" = \"2e8a\" ] || continue; [ -w \"$d\" ] || continue; ( hostname; ip -o -4 addr show dev wlan0 2>/dev/null || true; ip -o link show dev wlan0 2>/dev/null || true; ip -o -4 addr show dev eth0 2>/dev/null || true; ip -o link show dev eth0 2>/dev/null || true ) > \"$d\"; done; exit";
